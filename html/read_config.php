...
if($config['main']['mailer_mode'] == 'standalone'){
    ...
    echo ('<form name="user_config" id="user_config" action="save_config.php" onsubmit="return ValidateInput();" method="post">
           ...
           <tr align=left><td title="This is the customized list of IP addresses you wish to block.">Blacklist:</td><td><textarea form="user_config" id="blacklist" name="blacklist" rows=5 cols=70>'.$config['main']['blacklist'].'</textarea></td></tr>
           <tr align=left><td title="This is the list of IP addresses to be whitelisted from blocking by FalconGuard.">Whitelist:</td><td><textarea form="user_config" id="whitelist" name="whitelist" rows=5 cols=70>'.$config['main']['whitelist'].'</textarea></td></tr>
           <tr align=left><td title="This is the list of email addresses to be monitored for potential compromise due to hacking breaches in third party online services.">Email watchlist:</td><td><textarea form="user_config" id="email_watchlist" name="email_watchlist" rows=5 cols=70>'.$config['main']['email_watchlist'].'</textarea></td></tr>
           </table>
